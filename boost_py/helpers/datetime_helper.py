...
if __name__ == "__main__":
    ...
    if function == "current_ts":
        ...
        print("当前时间戳：%d" % DateTimeHelper.get_current_timestamp())
    elif function == "current_ds":
        # 当前日期时间戳 python3 datetime_utils.py current_ds
        print("当前日期的时间戳：%d" % DateTimeHelper.get_current_datestamp())
    elif function == "ts":
        ...
        fmt = sys.argv[3]
        print("%s的时间戳：%d" % (time_string, DateTimeHelper.get_timestamp(time_string=time_string, fmt=fmt)))
    elif function == "format_current_datetime":
        # 格式化当前时间戳
        if len(sys.argv) == 3:
            fmt = sys.argv[2]
            print("当前时间为：%s" % DateTimeHelper.get_current_formatted_datetime_string(fmt=fmt))
        else:
            print("当前时间为：%s" % DateTimeHelper.get_current_formatted_datetime_string())
        ...
        if len(sys.argv) == 4:
            fmt = sys.argv[3]
            print("%d的格式化时间为：%s" % (timestamp, DateTimeHelper.get_formatted_datetime_string(timestamp=timestamp, fmt=fmt)))
        else:
            print("%d的格式化时间为：%s" % (timestamp, DateTimeHelper.get_formatted_datetime_string(timestamp=timestamp)))
